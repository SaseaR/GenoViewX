...
#' @param focus_file Results file of FOCUS analysis
#' @param type Type of QTL
#' @param dis Disease name
#' @export



Mplot_for_TWAS_FOCUS <- function(file_path,file_name,title,Sig_FDR_Thresh,outDir,color_scheme,focus_file,type,dis){

  suppressMessages(library(data.table))
  suppressMessages(library(ggrepel))
  suppressMessages(library(ggplot2))
  suppressMessages(library(cowplot))
  focus_sig <- read.csv(focus_file)
  focus_sig$qtl_Type <- ifelse(focus_sig$qtl_Type=='eQTL','eqtl',ifelse(focus_sig$qtl_Type=='apaQTL','aqtl',ifelse(focus_sig$qtl_Type=='sQTL','sqtl',focus_sig$qtl_Type)))
  TWAS_manhattan = function(dataframe, title=title, color=color,ylimit=max(-log10(d$TWAS.P),na.rm=T)+1) {
    ...
    d_sig_all<-d_sig_all[order(d_sig_all$TWAS.FDR,d_sig_all$gene_name),]
    d_sig<-d_sig_all[!duplicated(d_sig_all$gene_name),]
    d_sig_all_left <- d_sig_all[!rownames(d_sig_all)%in%rownames(d_sig),]
    d_no_sig<-d[which(abs(d$TWAS.FDR) >= Sig_FDR_Thresh),]
    focus_sig <- subset(focus_sig,qtl_Type==type & phe.x==dis)
    focus_sig$gene_name <- gsub(' ','',focus_sig$gene_name,fixed = T)
    focus_sig <- subset(focus_sig,pips_pop1>0.5)
    ...
    if(nrow(d_sig) == 0){
      ...
      if(nrow(focus_sig)==0){
        p<-p+geom_text_repel(data=d_sig, aes(x=pos,y=-log10(d_sig$TWAS.P), label=gene_name),
                             colour='black', nudge_y=1, size=5, force=5, segment.alpha=0.25, ylim=c(sig_level+0.1,NA))
      } else{
        dat_lable <- subset(d_sig,gene_name%in%focus_sig$gene_name)
        p<-p+geom_text_repel(data=dat_lable, aes(x=pos,y=-log10(dat_lable$TWAS.P), label=gene_name),
                             colour='black', nudge_y=1, size=5, force=5, segment.alpha=0.25, ylim=c(sig_level+0.1,NA))
      }
      ...
      if(nrow(focus_sig)==0){
        ...
        dat_lable <- subset(d_sig,gene_name%in%focus_sig$gene_name)
        dat_no_lable <- subset(d_sig,!gene_name%in%dat_lable$gene_name)
        p<-ggplot(d_no_sig,aes(x=pos,y=-log10(d_no_sig$TWAS.P),colour=factor(CHR))) +
          geom_point(size=3) +
          scale_x_continuous(name="Chromosome", breaks=ticks, labels=chr_labs) +
          scale_y_continuous(name=bquote(-"log"[10]("p-value")),limits=c(0,ylimit)) +
          ...
          geom_hline(yintercept=sig_level,colour="black",linetype='dashed',size=1.5) +
          geom_point(data=d_sig_all_left, aes(x=pos,y=-log10(d_sig_all_left$TWAS.P)), colour="#F98232", fill='#F98232', size=3) +
          geom_point(data=dat_no_lable, aes(x=pos,y=-log10(dat_no_lable$TWAS.P)), colour="#DC0000B2", fill='#DC0000B2', size=3) +
          geom_point(data=dat_lable, aes(x=pos,y=-log10(dat_lable$TWAS.P)), colour="#DC0000B2", fill='#DC0000B2', size=3)
