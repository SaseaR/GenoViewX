...
Mplot_for_QTLs <- function(file_path,file_name,qtl_file,figure_pathy,figure_name,p_col,snp_col,p_threshold,colour){
  ...
  # dat_plot11 <- subset(dat_plot1,dat_plot1$SNP%in%qtl$variant_id)
  # dat_plot1$highlight <-
  highlight_snps <- dat_plot1$SNP[dat_plot1$SNP%in%qtl$variant_id]
  ...
  CMplot::CMplot(dat_plot1,plot.type="m",LOG10=TRUE,threshold.lwd=c(1,1), threshold.col='#BC3C29',main.font=1,ylab=expression(-log[10](italic(P))),
                 threshold=5e-04,threshold.lty=c(1,2),chr.den.col=NULL,col = cols,cex =c(0.5,0.5,0.5),lab.font=1,main="Chromosome",height = 4,width = 8.5,
                 amplify= TRUE, signal.cex=0.5, file="pdf",dpi=300,file.output=TRUE,verbose=TRUE,legend.pos='middle',  highlight = highlight_snps,  # 指定高亮
                 highlight.col = colour  )
